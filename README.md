...
出图内容：**每个账号一张卡片**、系统信息、CPU / 内存 / SWAP / GPU 资源环、磁盘占用、实时网速与累计流量、
...
配了几个号就画几张卡，**顺序按内核给出的账号顺序**，消息来自的那个号排在最前。
每张卡上的昵称、头像、在线状态、适配器名、好友与群数各是各的；整机数据（系统信息、
资源环、磁盘、网速、进程表）在账号卡之外**只画一遍**，不会跟着号数重复。

卡片上那个适配器名是**该账号实际在用的那一个**（取自账号记录里的 `adapterId`），
不是已安装的适配器列表 —— 装了 NapCat 与标准输入两个插件、而账号走的是标准输入时，
那一栏只写「标准输入」。
...
10. 配了两个以上账号时敲 `#状态pro` → 确认**每个号各占一张卡**，且每张卡上的名字、头像、
   在线状态、好友数各是各的；整机数据（系统信息、资源环、磁盘、网速）**只画一遍**，
   不在每张卡里重复
11. 在账号卡上确认**适配器那一栏只写该账号实际在用的那一个**（如只用标准输入时只写
   「标准输入」），不是把已安装的适配器全列出来
...
第 10、11 步各盯一个**只有多号或装了多个适配器时才显形**的缺陷：

- 第 10 步：状态图曾经只画一个账号（`pickAccount` 取"说话的那个"），多号使用者看不到
  另一个号，而图上毫无异常——只是少了一张卡。同时整机数据在模板里位于账号卡**之外**，
  故一张卡时不觉得，多张卡时才看得出它没有跟着重复。
- 第 11 步：适配器那一栏曾经取的是 `adapters.list()` 全体（那是面板「添加账号」页用来
  选类型的列表，与账号无关），于是装了几个适配器插件就显示几个名字——用标准输入的号上
  会并排写着「NapCat (OneBot v11) / 标准输入」。这一条**要装了第二个适配器才看得出来**，
  单适配器的机器上两种情况显示相同。
