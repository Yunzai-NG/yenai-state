...
进程负载、Redis 信息、fastfetch，以及 CPU / 内存 / 上下行网速 / 磁盘 IO 四条曲线。
...
不会留下一个空环。同理，未连 Redis、未装 fastfetch、磁盘速率取不到时，对应板块整块不出现。
...
单测覆盖的是纯函数（格式化、INFO 解析、三态开关、进程表排序、环形缓冲、模板翻译）；
采集要真机器、渲染要 Chromium，两者不做自动化测试，按下列步骤手工过一遍：
...
`redis.used_memory_human` 这些源自源插件内部结构的名字，与采集层定的名字对不上。改模板去
